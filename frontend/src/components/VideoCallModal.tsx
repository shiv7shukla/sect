...
import { Mic, MicOff, Video, VideoOff, PhoneOff, Lock, Maximize2, Volume2 } from "lucide-react";
...
interface Contact {
    ...
    const VideoCallModal = ({ isOpen, contact }: VideoCallModalProps) => {
    const { 
        callStatus, 
        isVideoOff, 
        isMuted, 
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => {if (remoteId) endCall(remoteId)}, 2500);
        return () => clearTimeout(t);
    }, [callStatus, endCall, remoteId]);
    ...
    return (
        ...
        <div className="h-full w-full flex flex-col">
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gradient-card">
                ...
                <h2 className="font-mono text-2xl font-semibold mb-1">{contact.displayName}</h2>
                <p className="text-muted-foreground text-sm font-mono">@{contact.username}</p>
                ...
                {callStatus === "declined" && (
                    ...
                    <span className="text-xs font-mono text-destructive">
                        {contact.displayName} declined the call
                    </span>
                    </div>
                )}
                </div>
