...
export const sendMessages = asyncHandler(async (req, res) => {
    ...
    if (senderId.toString() === receiverId)
        return res.status(400).json({ "message": "cannot send message to yourself" }); // Prevent sending messages to yourself
    const receiverObjectId = mongoose.Types.ObjectId.createFromHexString(receiverId);
    ...
    if (!message)
        return res.status(500).json({ "message": "unable to create new message" });
